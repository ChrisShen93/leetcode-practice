 ...
 */
pub struct Solution {}
use crate::helpers::link_list::{to_list, ListNode};
...
impl Solution {
    ...
    fn add_two_numbers_helper(
        ...
        let l1_node = l1.unwrap_or(Box::new(ListNode::new(0)));
        let l2_node = l2.unwrap_or(Box::new(ListNode::new(0)));

        let sum = l1_node.val + l2_node.val + carry;
        let mut current_node = ListNode::new(sum % 10);
        current_node.next = Solution::add_two_numbers_helper(l1_node.next, l2_node.next, sum / 10);
...
#[cfg(test)]
mod tests {
    use crate::solutions::s0002_add_two_numbers::Solution;
